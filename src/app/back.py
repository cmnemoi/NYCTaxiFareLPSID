...
def encode_values(sample: pd.DataFrame) -> pd.DataFrame:
    sample.loc[0, "VendorID"] = (
        1 if sample.loc[0, "VendorID"] == "Creative Mobile Technologies, LLC" else 2
    )
    sample.loc[0, "payment_type"] = (
        1 if sample.loc[0, "payment_type"] == "Carte de crédit" else 2
    )
...
def add_calculated_columns(sample: pd.DataFrame) -> pd.DataFrame:
    ...
    sample.loc[0, "airport_trip"] = (
        1
        if "Airport" in sample.loc[0, "PULocationLabel"]
        or "Airport" in sample.loc[0, "DOLocationLabel"]
        else 0
    )
...
def get_trip_distance(location_1: str, location_2: str) -> float:
    ...
    if distance_matrix_result["status"] != "OK":
        raise Exception(
            f"Une erreur s'est produite durant l'appel à l'API Google Maps : {distance_matrix_result['status']}"
        )

    distance = (
        distance_matrix_result["rows"][0]["elements"][0]["distance"]["value"] / 1000
    )  ##convert in km
